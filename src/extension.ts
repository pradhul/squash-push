...
const getCurrentBranch = async (workspaceFolder: string): Promise<string | null> => {
  try {
    return await execGitCommand("git symbolic-ref --short HEAD", workspaceFolder);
  } catch {
    return null;
  }
};
...
const getCurrentUpStreamBranch = async (currentBranch: string, workspaceFolder: string): Promise<string | null> => {
  try {
    return await execGitCommand(
      `git rev-parse --symbolic-full-name --abbrev-ref ${currentBranch}@{upstream}`,
      workspaceFolder
    );
  } catch {
    return null;
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const disposable = vscode.commands.registerCommand("squash-push.helloWorld", async () => {
    ...
    let localCommitLogsCmd: string = "git log --oneline";
    if (currentUpstreamBranch) {
      localCommitLogsCmd = `git log --oneline ${currentUpstreamBranch}..${currentBranch}`;
    }
    ...
    console.log("local commits are ", localCommits);
    ...
    try {
      ...
      try {
        execGitCommand(`git reset --soft ${commitID}~1 && git commit --amend`, workspaceFolder);
      } catch (err) {
      ...
      console.log("selected commit is ", commitID);
    } catch (err) {
